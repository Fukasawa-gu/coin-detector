...
def upload_multipart():
    try:
        if request.method == 'POST':
            ...
            os.makedirs(name + '/uploaded_files/', exist_ok=True)
            ...
            #print(os.listdir(name + '/uploaded_files/'))
            learn = load_learner('./', '20200126.pkl')

            trimed_folder = []
            detected_category = []
            s = ""
            for file in files:
                trimed = coin_detector(joined_path, file)
                trimed_folder.append(trimed)
                print(joined_path,',,,,,',file)
                p = joined_path+file.split('.')[0]+'/'
                p = p.replace('\\', '/')
                p = p.replace('./', '')
                print('detected:',p)
                for f in os.listdir(p):
                    predpath = p+f
                    open_image(predpath)
                    img = open_image(predpath)
                    detected_category = learn.predict(img)[0]

                    s += '<img src="' + '..\\uploaded_files\\'+time+'\\'+file.split('.')[0]+'\\'+f + '" height="84" width="84"/>'+ str(detected_category) +'</img>'
            print(s)
            ...
            #return render_template('index.html')#send_file('./uploaded_files/'+time+'.zip')
            with open('./templates/detected.html', mode='w', encoding="utf-8") as f:
                f.write(s)
